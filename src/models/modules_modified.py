...
class PostProcessingStandard(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        self.gn_conv_last = nn.Conv2d(256, 14, (1, 1), bias=False)

    def forward(self, x_out):
        out = self.gn_conv_last(x_out)
        return F.softmax(out, dim=1) #torch.sigmoid(out)
...
class GlobalNetwork(AbstractMILUnit):
    ...
    def __init__(self, parameters, parent_module):
        ...
        # downsampling-branch
        self.downsampling_branch = ResNetV2(input_channels=3, num_filters=16,
                 # first conv layer
...
class AttentionModule(AbstractMILUnit):
    ...
    def add_layers(self):
        ...
        # classifier
        self.parent_module.classifier_linear = nn.Linear(512, 14, bias=False)
    ...
    def forward(self, h_crops):
        ...
        y_crops = F.softmax(self.parent_module.classifier_linear(z_weighted_avg), dim=1)#torch.sigmoid(self.parent_module.classifier_linear(z_weighted_avg))
