...
from src.modelling import modules as m
import gin
import torch.nn.functional as F


def compute_cam_loss(model, parameters):
    """
    Function that computes the class activation map loss
    cam_loss = sum_{i,j} |cam_ij - prior|
    :param model:
    :param parameters:
    :return:
    """
    # prepare the prior vector
    N, C, _, _ = model.saliency_map.size()

    diff_map = model.saliency_map
    if parameters["cam_loss_l"] == "neg_log":
        epsilon = 0.05
        diff_map_distance = -(torch.log(diff_map+epsilon) - np.log(epsilon)) / np.log(epsilon)
    elif isinstance(parameters["cam_loss_l"], int) or isinstance(parameters["cam_loss_l"], float):
        diff_map_distance = torch.pow(torch.abs(diff_map), parameters["cam_loss_l"])
    # calculate the batch loss
    batch_cam_loss = diff_map_distance.mean()
    return batch_cam_loss
...
class GMIC(nn.Module):
    def __init__(self, parameters):
        ...
        # fusion branch
        self.fusion_dnn = nn.Linear(768, 14)
        #self.global_hg = nn.Linear(256, 14) 
